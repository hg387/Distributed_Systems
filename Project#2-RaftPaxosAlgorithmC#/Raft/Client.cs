...
namespace Raft.Client
{
    ...
    public class SimClient: UntypedActor, IWithTimers
    {
        ...
        public SimClient(List<IActorRef> nodes)
        {
            this.nodes = nodes;
            ...
            commitCmd.Add(new List<int>(){0, 7});
            
            this.nodes[0].Tell(new ClientRequest(cmd[0][1], Self));
            lastNodeSendTo = 0;
            lastCmdSendIndex = 0;
            Timers.StartSingleTimer("client", new Timeout(), TimeSpan.FromMilliseconds(2000));
        }
        ...
        protected override void OnReceive(object message)
        {
            switch (message)
            {
                case Timeout tout:
                    ...
                    this.nodes[lastNodeSendTo].Tell(new ClientRequest(cmd[lastCmdSendIndex][1], Self));
